...
```
You, in Apple's portals        Repo (automated)             You, in GitHub          CI (Mac runner)
─────────────────────────      ────────────────             ──────────────         ─────────────────
...
## Phase A — Apple side (only you can do; ~45 min)
...
4. **App Store Connect → app record** — My Apps → **+** → New App → iOS, the
   bundle ID, a name, SKU, primary language.
...
6. **Add yourself as an internal tester** — your app → TestFlight → Internal
   Testing → new group → add your Apple ID. No review; the build arrives minutes
   after it finishes processing.

---

## Phase B — Repo changes (DONE — see the PR that introduced this file)

These were the gaps that would otherwise have failed the upload or made the app
render wrong on device. All are unverifiable on Linux and are **confirmed by the
CI build itself** (watch the run after tagging):

- **App icon** — `MeltdownMonitor.iOS/Assets.xcassets/AppIcon.appiconset/`
  with a full-bleed, opaque **1024×1024** rendered from
  `assets/branding/app-icon.svg`, designated via `XSAppIconAssets` in
  `Info.plist`. (`.xcassets` are auto-included by the .NET iOS SDK, so no csproj
  change is needed. If `actool` ever fails to pick it up, add
  `<ImageAsset Include="Assets.xcassets\**\*" />` to the iOS csproj.)
...
- **Modern launch screen** — replaced the dangling `UILaunchStoryboardName`
...
- **CI toolchain pin** — the `testflight` job now selects **Xcode 26.0** to
...
## Phase C — GitHub secrets (only you can do; ~15 min)
...
**Environment `ios-release` secrets:**
...
`IOS_SIGNING_AVAILABLE == 'true'`. Once Phase B is merged and Phase C is set:
...
That fires `ios.yml`: `build-test` (the canary) + `testflight` (installs the iOS
workload, signs, archives the `.ipa`, and `altool` uploads to TestFlight).

---

## Phase E — On your iPhone

1. Install **TestFlight** from the App Store; sign in with the **same Apple ID**
   that you added as an internal tester.
2. After the build finishes *Processing* in App Store Connect (~10–30 min; the
   first one can take longer), open TestFlight → **Install**.
3. Launch → accept the first-run **disclaimer gate** → grant **Bluetooth** (and
   **HealthKit** when prompted) → pair the **Polar H10 / Verity Sense** →
   confirm the RR stream and that the regulation field moves.
...
- **iOS asset-catalog wiring is CI-verified**, since it can't be compiled on
  Linux. If the first run fails on the icon, the fallback is the explicit
  `<ImageAsset Include>` noted in Phase B.
