...
- **`MeltdownMonitor.Android.Services.HealthConnectStore` reads Health Connect for warm-start (Phase 5).** It pulls the last 24 h of `HeartRateRecord` via the managed `Xamarin.AndroidX.Health.Connect.ConnectClient` binding (the §11.1 question resolved in favour of the managed binding, not a JNI wrapper) to warm the EWMA HR baseline. Health Connect's `readRecords`/`getGrantedPermissions` are Kotlin `suspend` functions; the store bridges them to `Task` with a hand-rolled `Kotlin.Coroutines.IContinuation` (`SuspendContinuation`) and re-wraps the resumed handle with `JavaCast<T>()` (a `Java.Lang.Object` parameter won't pattern-match the concrete type). Every failure path (no provider, denied permission, IPC fault, 20 s timeout) degrades to a cold start — the pipeline tolerates an `IHealthStore` that yields nothing. `RequestAuthorizationAsync` only reports whether the read grant already exists; launching Health Connect's permission UI (an `ActivityResultContract`) is Phase 4 permission-sequencing work. `WriteEpisodeAsync` (Phase 8, landed) records each opt-in alert as an "other workout" `ExerciseSessionRecord` via `InsertRecords` over the same suspend bridge, gated upstream by `HealthKitEpisodeRecorder` on `WriteEpisodesToHealthKit` (default off); the `WRITE_EXERCISE` grant still depends on the pending permission-UI launcher, so until then the write is a silent no-op. `WriteHrSampleAsync` stays an intentional no-op (never called by the pipeline). The read/write behaviour is only verifiable on a device with Health Connect installed.
