...
- **No per-region blend modes on the Desktop (ImGui) head.** The `ktsu.ImGui.App` OpenGL renderer `throw`s `NotImplementedException` on any draw-command `UserCallback`, and sets the blend func once globally per frame. So the usual ImGui trick of `ImDrawList.AddCallback` to flip `glBlendFunc` (e.g. additive `SRC_ALPHA, ONE`) for a region and reset it after will crash, not blend. Anything needing additive/glow blending (e.g. the regulation field) requires an upstream renderer change in `ktsu.ImGui.App` — see `docs/regulation-field-blend-modes.md`.
